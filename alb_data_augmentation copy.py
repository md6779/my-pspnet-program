...
def save_process(transformed_data: dict[str, Any], save_root: Path, stem: str) -> None:
    ...
    cv2.imwrite(str(save_root / f"{stem}.png"), tfd_mask)
...
def noise_img():
    ...
    root = Path(r"D:\img_out\noflip\Original")
    output_path = Path(r"D:\resized_augmented_5_noflip")
...
def main():
    ...
    for file_path in root.glob("*.jpg"):
        ...
        mask_path = file_path.parent.parent / f"Annotated/{file_path.stem}.png"
        seg = cv2.imread(str(mask_path))
        print(mask_path)

        base_transform_data = {"image": img, "mask": seg}
        ...
        if base_transformed is not None:
            ...
            save_process(
                transformed_data={"image": snp_noise, "mask": base_transform_data["mask"]}, 
                save_root=output_path, 
                stem=f"{file_path.stem}_snp_noise"
                )
            save_process(
                transformed_data={"image": poisson, "mask": base_transform_data["mask"]}, 
                save_root=output_path, 
